...
yaml2graph is a **dynamic code generation system** that creates knowledge graph from YAML configuration files. 

1- Registry module:Core part of the yaml2graph is Registry module which is developed based on registry design pattern. Think of it as a code factory that reads configuration which is in this case is Yaml file and builds classes automatically. 
...
1. Configuration Files (`yaml2graph/config/` folder)
...
2. Registry Loading (`yaml2graph/registry/loaders.py`)
...
3. Validation (`yaml2graph/registry/validators.py`)

- Checks that all required sections exist
- Validates configuration structure
- Ensures everything is properly configured

4. Code Generation (`yaml2graph/registry/generators.py`)
...
5. Class Generation (`yaml2graph/registry/writers.py`)
...
6. Factory (`yaml2graph/registry/factory.py`)
