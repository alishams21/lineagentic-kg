...
def main():
    # Create factory
    factory = RegistryFactory("refactoring/enhanced_registry.yaml")
    ...
    writer = factory.create_writer(uri, user, password)
    
    # Now you can use the writer with generated methods
    try:
        # Example usage
        dataset_urn = writer.upsert_dataset(platform="snowflake", name="my_table", env="PROD")
        user_urn = writer.upsert_corpuser(username="john.doe@company.com")
        
        # Debug: Print available methods
        print("Available methods:")
        for method in dir(writer):
            if method.startswith('create_') and method.endswith('_relationship'):
                print(f"  {method}")
        
        # Create the relationship using the specific method
        print(f"\nCreating relationship between {user_urn} and {dataset_urn}")
        writer.create_owns_corpuser_to_dataset_relationship(user_urn, dataset_urn, {"via": "aspect"})
        
        # Create aspects using the new aspect methods
        print("\nCreating aspects for Dataset...")
        
        # Create schemaMetadata aspect (versioned) - now with validation
        schema_payload = {
            "schemaName": "snowflake.my_table",
        ...
        # Create datasetProperties aspect (versioned) - now with validation
        properties_payload = {
        ...
        writer.upsert_datasetproperties_aspect("Dataset", dataset_urn, properties_payload)
        
        # Create ownership aspect (versioned) - now with validation
        ownership_payload = {
            "owners": [
                {
                    "owner": user_urn,
        ...
        # Create datasetProfile aspect (timeseries) - now with validation
        profile_payload = {
            "rowCount": 1000,
            "columnCount": 3,
            "sizeInBytes": 10240,
            "lastModified": writer.utc_now_ms()
        }
        writer.upsert_datasetprofile_aspect("Dataset", dataset_urn, profile_payload)
        
        # Create aspects for the CorpUser using the new aspect methods
        print("\nCreating aspects for CorpUser...")
        
        # Create corpUserInfo aspect (versioned) - now with validation
        user_info_payload = {
            "displayName": "John Doe",
            "email": "john.doe@company.com",
            "title": "Data Engineer",
            "department": "Engineering"
            # Note: 'active' will be set to True by default from registry
        }
        writer.upsert_corpuserinfo_aspect("CorpUser", user_urn, user_info_payload)
        
        print("Successfully created entities, relationships, and aspects!")
        print(f"Dataset URN: {dataset_urn}")
        print(f"User URN: {user_urn}")
        
        # Verify the entities and aspects were created
        with writer._driver.session() as s:
            print("\n=== Verifying Dataset and its aspects ===")
            result = s.run("""
                MATCH (d:Dataset {urn: $dataset_urn})
                OPTIONAL MATCH (d)-[:HAS_ASPECT]->(a:Aspect)
                RETURN d.urn as dataset_urn, 
                       collect({name: a.name, kind: a.kind, version: a.version}) as aspects
            """, dataset_urn=dataset_urn)
            for record in result:
                print(f"Dataset: {record['dataset_urn']}")
                for aspect in record['aspects']:
                    if aspect['name']:
                        print(f"  - {aspect['name']} ({aspect['kind']}) v{aspect['version']}")
            
            print("\n=== Verifying CorpUser and its aspects ===")
            result = s.run("""
                MATCH (u:CorpUser {urn: $user_urn})
                OPTIONAL MATCH (u)-[:HAS_ASPECT]->(a:Aspect)
                RETURN u.urn as user_urn, 
                       collect({name: a.name, kind: a.kind, version: a.version}) as aspects
            """, user_urn=user_urn)
            for record in result:
                print(f"User: {record['user_urn']}")
                for aspect in record['aspects']:
                    if aspect['name']:
                        print(f"  - {aspect['name']} ({aspect['kind']}) v{aspect['version']}")
            ...
            result = s.run("""
                ...
                RETURN u.urn as user_urn, d.urn as dataset_urn, r.via as via
                LIMIT 5
            """)
            for record in result:
                print(f"Found relationship: {record['user_urn']} -[OWNS]-> {record['dataset_urn']} (via: {record['via']})")
        
        # Create multiple versions of an aspect
        print("\n=== Creating multiple versions of schemaMetadata ===")
        ...
        schema_v1 = {
            ...
            "platform": "urn:li:dataPlatform:snowflake",
            "fields": [
                {"fieldPath": "id", "type": {"type": "INTEGER"}, "description": "Primary key", "nullable": False},
                {"fieldPath": "name", "type": {"type": "VARCHAR"}, "description": "Name field", "nullable": True}
            ]
        }
        writer.upsert_schemametadata_aspect("Dataset", dataset_urn, schema_v1)

        # Version 2 (add a new field)
        schema_v2 = {
            "schemaName": "snowflake.my_table",
            "platform": "urn:li:dataPlatform:snowflake",
            "fields": [
                {"fieldPath": "id", "type": {"type": "INTEGER"}, "description": "Primary key", "nullable": False},
                {"fieldPath": "name", "type": {"type": "VARCHAR"}, "description": "Name field", "nullable": True},
                {"fieldPath": "created_at", "type": {"type": "TIMESTAMP"}, "description": "Creation timestamp", "nullable": False}
            ]
        }
        writer.upsert_schemametadata_aspect("Dataset", dataset_urn, schema_v2)

        # Version 3 (modify description)
        schema_v3 = {
            "schemaName": "snowflake.my_table",
            "platform": "urn:li:dataPlatform:snowflake",
            "fields": [
                {"fieldPath": "id", "type": {"type": "INTEGER"}, "description": "Primary key identifier", "nullable": False},
                {"fieldPath": "name", "type": {"type": "VARCHAR"}, "description": "User name field", "nullable": True},
                {"fieldPath": "created_at", "type": {"type": "TIMESTAMP"}, "description": "Record creation timestamp", "nullable": False}
            ]
        }
        writer.upsert_schemametadata_aspect("Dataset", dataset_urn, schema_v3)

        # Query version history
        print("\n=== Schema Metadata Version History ===")
        history = writer.get_aspect_version_history("Dataset", dataset_urn, "schemaMetadata")
        for version in history:
            print(f"Version {version['version']} (Latest: {version['is_latest']}) - Created: {version['created_at']}")
            print(f"  Fields: {len(version['payload']['fields'])}")
            for field in version['payload']['fields']:
                print(f"    - {field['fieldPath']}: {field['type']['type']}")

        # Get latest version
        print("\n=== Latest Schema Metadata ===")
        latest = writer.get_latest_aspect_version("Dataset", dataset_urn, "schemaMetadata")
        if latest:
            print(f"Latest version: {latest['version']}")
            print(f"Created: {latest['created_at']}")

        # Get specific version
        print("\n=== Version 1 Schema Metadata ===")
        v1 = writer.get_aspect_version("Dataset", dataset_urn, "schemaMetadata", 1)
        if v1:
            print(f"Version {v1['version']} - Fields: {len(v1['payload']['fields'])}")
