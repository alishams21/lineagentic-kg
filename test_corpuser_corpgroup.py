...
print("🔍 Testing CorpUser-CorpGroup relationship creation")
...
with neo4j_writer._driver.session() as s:
    result = s.run("""
        ...
        RETURN u.urn as user_urn, g.urn as group_urn, type(r) as relationship_type, r.added_via as added_via
    """)
    ...
    if relationships:
        ...
        for rel in relationships:
            print(f"   {rel['user_urn']} -> {rel['relationship_type']} -> {rel['group_urn']} (added_via: {rel['added_via']})")
    else:
